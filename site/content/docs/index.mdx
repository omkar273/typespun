...
description: Install Typespun, generate your first validated loader, and see the exact output of a successful run and a startup failure.
---

In about five minutes you will declare a configuration as a TypeScript
interface, generate a validated loader, run it, and see what a startup failure
looks like. The tutorial uses Bun; the generated loader also runs on Node.js 22
and 24.

Want to try it first? [Open the playground](/playground) and edit a declaration
with nothing to install.

## Prerequisites

- Bun 1.4.1 or newer (or Node.js 22 or newer, adjusting the commands)
- An ESM TypeScript project with a `package.json`
- A usable `tsconfig.json` that includes the schema and generated module

Typespun is pre-1.0. Pin both package versions if you need reproducible builds.
...
pnpm exec typespun init --env-prefix APP`}
/>
...
<Install
  bun="bun run config:check"
  npm="npm run config:check"
  pnpm="pnpm run config:check"
/>
...
<Cards>
  <Card
    ...
    description="The five source kinds, resolved per leaf, highest defined value wins."
  />
  <Card
    title="Generated code"
    href="/docs/concepts/generated-code"
    description="Why the loader is committed, how determinism works, and what check does in CI."
  />
  <Card
    title="CLI reference"
    href="/docs/reference/cli"
    description="init, generate and check — flags, output and exit codes."
  />
  <Card
    title="Runnable examples"
    href="https://github.com/omkar273/typespun/tree/main/examples"
    description="The interface and class examples this tutorial mirrors, exercised by CI."
  />
</Cards>
